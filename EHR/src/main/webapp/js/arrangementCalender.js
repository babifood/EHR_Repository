...
function drawCld(SY, SM) {
    ...
    $.ajax({
        ...
        type:'POST',
        data:{"year":SY,'month':SM+1,arrangementId:arrangementId},
        cache:false,
        async:false,
        dataType:'json',
...
//            			lObj.innerHTML = "<span style='color:red'>"+time+"</span>";
    ...
    $("table.biao .calendar_day").click(function() {
    	...
    	arrangementNode = this;
    	arrangementNode.classList.add("selday");
	})
}

function addSpecialArrangement(){
	$("#arrangement_type").text("新增特殊排班");
	$("#arrangement_edit").dialog("open").dialog("center").dialog("setTitle","特殊排班");
	$("#arrangement_form").form("clear");
	$("#attendance_remark").val("");
	if(arrangementNode){
		var year = global.currYear;
		var month = global.currMonth;
		var day = arrangementNode.firstChild.innerHTML;
		var data = year+"-"+((month+1)>=10?(month+1):"0"+(month+1))+"-"+(day>=10?day:"0"+day);
		$("input[name='date']").val(data);
		$("#attendance_datebox").parent().find(".textbox-text").val(data);
		$("#attendance_datebox").val(data);
	}
	$("#attendance_isAttend").combobox({
		valueField:'id',    
	    textField:'text',
	    data:[
	    	{"id":0, "text":"不考勤"},
			{"id":1, "text":"考勤",selected:true}
	    ],
	    onSelect:function(record){
	    	if(record.id == 0){
	    		$(".attendance_time").hide();
	    		$("#arrangement_edit").dialog("close").dialog("open");
	    	} else if (record.id == 1) {
	    		$(".attendance_time").show(); 
	    		$("#arrangement_edit").dialog("close").dialog("open");
	    		$('#attendance_startTime').timespinner('setValue', '07:00');
	    		$('#attendance_endTime').timespinner('setValue', '16:00');
			}
	    }
	})
}
...
function updateSpecialArrangement(){
	...
	if(id){
		$.ajax({
			url:"arrangement/base/findSpecialArrangement?id="+id,
			success:function(data){
				if(data.code=="1"){
					$("#arrangement_type").text("修改特殊排班");
					$("#arrangement_edit").dialog("open").dialog("center").dialog("setTitle","特殊排班");
					$("#arrangement_id").val(data.arrangement.id);
					$("input[name='date']").val(data.arrangement.date);
					$("#attendance_datebox").parent().find(".textbox-text").val(data.arrangement.date);
					$("#attendance_datebox").val(data.arrangement.date);
					if(data.arrangement.isAttend == 1){
						$('#attendance_startTime').timespinner('setValue', data.arrangement.startTime);
			    		$('#attendance_endTime').timespinner('setValue', data.arrangement.endTime);
					}
					$("#attendance_remark").val(data.arrangement.remark);
					$("#attendance_isAttend").combobox({
						valueField:'id',    
					    textField:'text',
					    data:setAttendValue(data.arrangement.isAttend),
					    onSelect:function(record){
					    	if(record.id == 0){
					    		$(".attendance_time").hide();
					    		$("#arrangement_edit").dialog("close").dialog("open");
...
function setAttendValue(isAttend){
	if(isAttend == 1){
		return [{"id":0, "text":"不考勤"},{"id":1, "text":"考勤",selected:true}]
	}
	return [{"id":0, "text":"不考勤",selected:true},{"id":1, "text":"考勤"}]
}
...
function removeSpecialArrangement(){
	...
	var id = arrangementNode.getAttribute("data_id");
	if(id){
		$.messager.confirm('确认对话框', '确定删除特殊排班吗？', function(b){
			if(b){
				$.ajax({
					url:"arrangement/base/removeSpecialArrangement?id="+id,
					success:function(data){
						$.messager.show({
							title:"提示消息",
							msg:data.msg,
...
//保存特殊排班
function saveSepcialArrangement(){
	var id = $("#arrangement_id").val();
	var date = $("#attendance_datebox").val();
	var isAttend = $("#attendance_isAttend").val();
	var startTime = $("#attendance_startTime").val();
	var endTime = $("#attendance_endTime").val();
	var remark = $("#attendance_remark").val();
	var url = "arrangement/base/saveSpecialArrangement";
	$.ajax({
		url:url,
		type:'POST',
		data:{
...
function arrangeInit(id) {
    ...
    /*年份递减*/
    $("#nianjian").unbind("click").click(function() {
        dateSelection.goPrevYear();
    });
    /*年份递加*/
    $("#nianjia").unbind("click").click(function() {
        dateSelection.goNextYear();

    });

    /*月份递减*/
    $("#yuejian").unbind("click").click(function() {
        dateSelection.goPrevMonth();
        $('.holiday').show();
    });
    
    /*月份递加*/
    $("#yuejia").unbind("click").click(function() {
